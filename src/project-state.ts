...
export type ProjectTrackingState = "unknown" | "opted-in" | "opted-out" | "initialized";

export interface ProjectTrackingStateEntry {
  path: string;
...
interface ProjectTrackingStateFile {
  schemaVersion: 1;
  updatedAt: string;
  projects: Record<string, ProjectTrackingStateEntry>;
}

function defaultHomeDir(): string {
  return (
    process.env.AWESOME_PROGRESS_TRACKER_HOME ??
    process.env.PLUGIN_DATA ??
    path.join(os.homedir(), ".awesome-progress-tracker")
  );
}

export function projectStatePath(options: ProjectStateOptions = {}): string {
...
function unknownEntry(projectPath: string): ProjectTrackingStateEntry {
  return {
...
async function readStateFile(options: ProjectStateOptions = {}): Promise<ProjectTrackingStateFile> {
  const filePath = projectStatePath(options);
  try {
    const raw = await fs.readFile(filePath, "utf-8");
    const parsed = JSON.parse(raw) as ProjectTrackingStateFile;
...
async function writeStateFile(stateFile: ProjectTrackingStateFile, options: ProjectStateOptions = {}): Promise<void> {
  const filePath = projectStatePath(options);
  await fs.mkdir(path.dirname(filePath), { recursive: true });
  await fs.writeFile(filePath, `${JSON.stringify(stateFile, null, 2)}\n`, "utf-8");
}

export async function readProjectTrackingState(
  projectPath: string,
  options: ProjectStateOptions = {}
): Promise<ProjectTrackingStateEntry> {
...
export async function listProjectTrackingStates(
  options: ProjectStateOptions = {}
): Promise<ProjectTrackingStateEntry[]> {
  const stateFile = await readStateFile(options);
...
export async function resetProjectTrackingState(
  projectPath: string,
  options: ProjectStateOptions = {}
): Promise<boolean> {
  const stateFile = await readStateFile(options);
